...
@section('konten')
  ...
  <div class="container" style="margin-top:30px" align="center">
    ...
    <div class="row featurette">
      <div class="col-md-7">
        <h2 class="featurette-heading">Struktur Karyawan <span class="text-muted">Departement Audit.</span></h2>
    ...
    <div class="row featurette">
      ...
      <div class="col-md-7" >
        <h2 class="featurette-heading">Visi dan Misi <span class="text-muted">Departemen Audit.</span></h2>
      ...
      <table class="table table-hover">
        ...
        <tbody>
          ...
          <tr>
            <td>{{ $temp->id }}</td>
            <td>{{ $temp->nama }}</td>
            <td><a href="{{ asset('files/audit/sop') }}/{{ $temp->file_sop }}">{{ $temp->file_sop }}</a></td>
            <td>{{ $temp->keterangan }}</td>
          </tr>
          @endforeach
        </tbody>
      </table>
      <a href="{{ route('auditsop') }}" class="btn btn-info pull-right" >Read More...</a>
    </div>
        ...
        <tbody>
          @foreach($laporan as $temp)
            <tr>
              <td>{{ $temp->id }}</td>
              <td>{{ $temp->nama }}</td>
              <td><a href="{{ asset('files/audit/laporan') }}/{{ $temp->file_laporan }}">{{ $temp->file_laporan }}</a></td>
              <td>{{ $temp->keterangan }}</td>
            </tr>
            @endforeach
        </tbody>
      </table>
      <a href="{{ route('auditlaporan') }}" class="btn btn-info pull-right" >Read More...</a>
    </div>
